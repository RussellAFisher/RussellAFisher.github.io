$(document).ready(function() {
    ...
    $(".save, .nextButton").click(function(event) {
        ...
        if (earthDate === "") {
            $.get("https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?sol=" + marsDate + "&camera=" + cameraAngle + "&api_key=t5rTsXuOkLjqX0VKm3ZDXDF0pUghwSUtTB5Azlgf").done(function(data) {
            ...
            $.get("https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?earth_date=" + earthDate + "&camera=" + cameraAngle + "&api_key=t5rTsXuOkLjqX0VKm3ZDXDF0pUghwSUtTB5Azlgf").done(function(data) {
                console.log(data);
                roverArray = (data.photos);
                if (arrayIndex < roverArray.length) {
                    getEarthDate = data.photos[arrayIndex].earth_date;
